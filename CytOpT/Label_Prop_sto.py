...
def Robbins_Wass(X_s, X_t, alpha, beta, eps=0.0001, const=0.1, n_iter=10000):
    ...
    for k in range(n_iter):
        ...
        # Computation of the gradient if eps>0 or sub-gradient if eps=0.
        if eps == 0:
...
def Label_Prop_sto(L_source, f, X, Y, alpha, beta, eps=0.0001):
    ...
    :return:
    L_target: np.array of shape (K,n_obs_target), where K is the number of different type of cell populations in the source
    data set. The coefficient L_target[k,j] corresponds to the probability that the observation X_t[j] belongs to the
    class k.
    clustarget: np.array of shape (n_obs_target,). This array stores the optimal transport based classification of the
    target data set.
    """
    print(alpha)
    J = Y.shape[0]
